"""
🌙 Moon Dev's Derivatives Monitor Subagent
Part of the Hedge Monitor System
Built with love by Moon Dev 🚀

Monitors Open Interest, Funding Rates, and Liquidations for market risk assessment.
"""
...
class DerivativesMonitorSubagent(BaseAgent):
    """Derivatives Monitor - Tracks OI, funding rates, and liquidations"""

    def __init__(self):
        """Initialize Derivatives Monitor Subagent"""
        super().__init__('derivatives_monitor')
        ...
        # History files
        self.funding_history_file = self.data_dir / "funding_history.csv"
        self.oi_history_file = self.data_dir / "oi_history.csv"
        self.liquidation_history_file = self.data_dir / "liquidation_history.csv"

        self.load_history()

        cprint("📊 Derivatives Monitor Subagent initialized!", "white", "on_blue")
    ...
    def get_funding_rates(self):
        ...
        try:
            ...
            if df is not None and not df.empty:
                ...
                current_data['funding_rate'] = pd.to_numeric(current_data['funding_rate'], errors='coerce')
                current_data['yearly_funding_rate'] = pd.to_numeric(
                    current_data['yearly_funding_rate'], errors='coerce'
                )

                # Rename for consistency
                current_data = current_data.rename(columns={'yearly_funding_rate': 'annual_rate'})

                print(f"✅ Got funding rates for {len(current_data)} symbols")
                return current_data
        ...
        try:
            ...
            if df is not None and not df.empty:
                ...
                oi_data = {
                    ...
                    'total_oi': latest.get('total_oi', 0),
                    'btc_oi': latest.get('btc_oi', 0),
                    'eth_oi': latest.get('eth_oi', 0)
                }
    ...
    def get_liquidations(self):
        ...
        try:
            ...
            df = self.api.get_liquidation_data(limit=10000)
    ...
    def run(self):
        ...
        try:
            ...
            # Print summary
            print("\n" + "╔" + "═" * 70 + "╗")
            print("║            🌙 Derivatives Monitor Summary 📊                       ║")
            print("╠" + "═" * 70 + "╣")
...
def main():
    ...
    cprint("\n🚀 Derivatives Monitor Subagent Starting...", "white", "on_blue")

    monitor = DerivativesMonitorSubagent()
